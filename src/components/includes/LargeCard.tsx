...
export default function LargeCard({
	...
	const addtoCart = async (updatedQuantity: any) => {
		...
		try {
			...
			const response = await fetch(
				`${API_URL}carts/add-cart-item/${product?.product?.slug}/`,
				{
					method: "POST",
					headers,
					body: JSON.stringify({
	...
	return (
		<>
			...
			<Modal
				...
				className=" p-8">
				<ReturnReasonModal product={product} />
			</Modal>
			<Modal
				isOpen={cancelModal}
			...
			<div
				onClick={onClick}
				className={cn(
					"relative  cursor-pointer block w-[100%] mt-[12px]  max-lg:mt-0 py-[16px] border-t border-solid border-Platinum hover:opacity-[0.9]",
					myReviews && "mt-0, border-0 opacity-[1] py-0",
					myOrder && "mt-0, border-0 opacity-[1] py-0",
					ischeckOut && "border border-solid rounded-[4px] px-3 py-2"
				)}>
				<div
					className={cn(
						"items-start  py-[16px] w-[100%] flex lg:flex-row flex-col justify-between ",
						ischeckOut && "py-0 items-center"
					)}>
					<div
						className={cn(
							"flex flex-row  lg:w-[71.42%] ",
							ischeckOut && "items-center"
						)}>
						<div className="w-[96px] h-[104px] min-w-[96px] max-sm:self-center flex   justify-center items-center">
							<Image
								src={product?.product?.primary_attachment}
								width={100}
								height={100}
						...
						<div
							className={cn(
								"sm:mx-[16px] w-[100%]  flex flex-col justify-between",
								ischeckOut ? "h-auto" : ""
							)}>
							<div className="">
								<h1 className="rubik_medium  md:text-[16px] text-[12px] sm:text-[14px] leading-[20px] md:leading-[24px] line-clamp-3 text-start text-[shadow_gray]">
									{product?.product?.name}
								</h1>
								{ischeckOut && (
									<h1 className="rubik_medium text-[14px] mk:text-[16px] leading-[24px] text-start text-[shadow_gray]">
							...
							{myOrder && (
								<div className="flex flex-col w-full">
									...
									{product?.return_policy_end_date && (
										<div className="text-[#717171] rubik_regular text-[14px]">
					...
					<div className=" flex   items-center ">
						...
						{myOrder && product?.current_status !== "closed" && (
				...
				{myReviews && (
					<div className="w-full border-t border-solid border-Platinum py-2">
						<div className="w-full flex flex-col">
							<div className="w-full flex justify-between mt-2">
								<div className="rubik_medium text-[16px] text-[#1d252c]">
									{product?.title}
								</div>
								<div>
									<StarRatings
							...
							<div className="rubik_regular mt-2 text-[12px]">
								{product?.review}
							</div>
						</div>
				...
				{!myOrder && !myReviews && (
					<div className="flex flex-row items-end justify-end gap-[20px] sk:gap-[40px] ">
						{!wishlist && !ischeckOut && (
							<div>
								<UnderLinedButton
									onClick={(e: any) => {
									...
									title={
										!wishlist
											? Strings.button.moveToWishlist
											: Strings.button.moveToCart
									}
								/>
							</div>
						)}
						...
						{product?.inStock && (
							<div>
								<UnderLinedButton
									...
									title={
										wishlist
											? Strings.button.addCart
											: Strings.button.saveLater
									}
								/>
							</div>
						)}
						{!wishlist &&
							!ischeckOut &&
							product?.product?.stock_status?.status !=
								"not available" && (
								<QuantityButton
									isLoading={isLoading}
