...
export default function Page() {
	...
	const getData = async () => {
		const response = await fetchApiData<any>(
			`products/list-products?categories=powered-by-asus&page=${page}&per_page=10`
		);
		...
		<div>
			<Wrapper className="max-xl:pt-[125px] max-mc:px-4 max-sm:px-2 max-sl:px-1 max-sk:px-[2px]  ">
				<div className="max-[480px]:px-[15px] w-full">
					...
					<section className="bg-[#1c1c1c] text-white py-8 rounded-lg ">
						<div className="container mx-auto px-4">
							...
							<p className="text-lg">
								Powered by ASUS is a global program across more
								than 40 countries and involves over 500 partners
								who provide the very best customized systems.
								These tailored rigs feature a best-selling ASUS
								motherboard and graphics card, with a wide array
								of industry-leading gaming monitors, routers,
								CPU coolers, power supplies, chassis, and
								peripherals available for a seamless fusion of
								synchronized features and tuned performance.
							</p>
						</div>
					</section>
				...
				<ProductSection
					...
					cardStyle="w-[19%] max-lgs:w-[24%] mb-2 max-mdx:w-[32%] max-sm:w-[49%]"
					title="Available PBA PCs"
				/>
			</Wrapper>
		</div>
	);
