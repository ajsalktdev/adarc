...
export default function MobileBanner({ data }: { data: any[] }) {
  ...
  useEffect(() => {
    const interval = setInterval(scrollBanner, 4000); // Slightly slower than 1s
    return () => clearInterval(interval);
  }, [direction]);
  ...
  return (
    <div
      ref={mobileRef}
      className="flex sm:hidden overflow-x-scroll no-scrollbar w-full snap-x snap-mandatory"
    >
      {data?.map((item, index) => (
        <Link
          href={item?.link || '#'}
          key={index}
          className="w-full min-w-full snap-start relative aspect-[2/1] flex-shrink-0"
        >
          <Image
            src={item?.image}
            alt={`banner-${index}`}
